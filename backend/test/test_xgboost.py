...
def test_successful_prediction(client, predict_xgboost_url, valid_payload, ml_models_path):
    """
    Tests the 200 OK scenario where the prediction service returns a result.
    """
    with mock.patch(ml_models_path, return_value=5) as mock_predict:
...
def test_prediction_service_unavailable_503(client, predict_xgboost_url, valid_payload, ml_models_path):
    """
    ...
    with mock.patch(ml_models_path, side_effect=RuntimeError(error_message)):
        response = client.post(predict_xgboost_url, json=valid_payload)
...
def test_internal_server_error_500(client, predict_xgboost_url, valid_payload, ml_models_path):
    """
    ...
    with mock.patch(ml_models_path, side_effect=ValueError(error_message)):
        response = client.post(predict_xgboost_url, json=valid_payload)
